 ...
 * Input parameters: Note that this program is only intended to be
   ...
   p3main does the argument validation.
     ...
     o argv[3] is a string representation of the nonnegative
...
int mainChange(int argc, char *argv[]) {
  ...
  assert( 4 == argc ); /* p3robot assumes validation is done before
  ...
  if (SEM_FAILED == done){
    fprintf(stderr,"Semaphore initialization failed in p3robot.c -- exiting\n");
    exit(3);
  }
  CHK(sem_post(done));  
